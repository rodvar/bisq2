...
@Slf4j
public class MuSigArbitratorView extends View<ScrollPane, MuSigArbitratorModel, MuSigArbitratorController> {
    ...
    private final MuSigArbitrationTableView muSigArbitrationTableView;
    ...
    public MuSigArbitratorView(MuSigArbitratorModel model,
                               MuSigArbitratorController controller,
                               HBox arbitrationCaseHeader,
                               VBox chatMessagesComponent) {

        super(new ScrollPane(), model, controller);

        muSigArbitrationTableView = new MuSigArbitrationTableView(model, controller);
        ...
        arbitrationCaseHeader.getChildren().add(toggleChatWindowButton);
        ...
        chatVBox = new VBox(arbitrationCaseHeader, Layout.hLine(), chatContentSlot);
        chatVBox.setAlignment(Pos.CENTER);
        chatVBox.getStyleClass().add("bisq-easy-container");

        VBox.setVgrow(chatVBox, Priority.ALWAYS);
        VBox.setMargin(muSigArbitrationTableView, new Insets(0, 0, 10, 0));
        centerVBox = new VBox(muSigArbitrationTableView, chatVBox);
    ...
    protected void onViewAttached() {
        muSigArbitrationTableView.initialize();
    ...
    protected void onViewDetached() {
        muSigArbitrationTableView.dispose();
